...
\begin{document}
	...
	\begin{align}
		\visible<1->{A[n] = A[n, \phi_j[n]] = &T\s[\phi_j[n]] - k_\mathrm{B}\tau S\s[\phi_j[n]] + E\h[n] + E\xc[n] + E^{ei}[n] \nonumber \\}
		\visible<2->{d(\epsilon, \br) = &\sum_j {\mid\phi_j(\br)\mid}^2 \delta(\epsilon-\epsilon_j) \nonumber \\}
		\visible<3->{n(\br) = & \int d \epsilon\;  f^\tau(\epsilon) d(\epsilon, \br) \; \nonumber \\
	        D(\epsilon) =  &\int d\br \; d(\epsilon, \br)  \nonumber \\}
		\visible<4->{A[n] = A[n[d], D[d]] = &E_b[D[d]]-\kB\tau S_\mathrm{S}[D[d]]-E_\mathrm{H}[n[d]] \nonumber +E_\mathrm{XC}[n[d]]-\int d \br v_\mathrm{XC}(\br)n[d](\br) \\}
		\visible<5->{\tilde{d}(\epsilon, \br) = &M(B(\,J, \br)) \nonumber }
	\end{align}
	\end{frame}
